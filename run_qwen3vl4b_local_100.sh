...
RUN_ROOT="${RUN_ROOT:-$ROOT/tmp/qwen3vl4b_local_100}"
...
INPUT_SAMPLE_NUM="${INPUT_SAMPLE_NUM:-140}"
TARGET_IMAGES="${TARGET_IMAGES:-100}"
...
"$AGENT_PYTHON" - "$OUTPUT_ROOT" "$IMAGE_OUTPUT_DIR" "$GPU_LOG" "$SUMMARY_PATH" "$TARGET_IMAGES" "$((END_EPOCH - START_EPOCH))" <<'PY'
import csv
import json
import shutil
import sys
