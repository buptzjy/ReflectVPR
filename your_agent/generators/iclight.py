...
DEFAULT_START_SCRIPT = "/media/data/zhangjingyi/IC-Light/start_server.sh"
...
def _start_local_service(session: requests.Session, api_url: str) -> None:
    ...
    print(f"[ICLight] 未检测到服务，自动启动: {script}")
    ...
    while time.time() < deadline:
        if _service_ready(session, api_url):
            print(f"[ICLight] 服务已就绪: {api_url}")
            return
        time.sleep(2)
    print(f"[ICLight] 服务启动超时，日志: {log_path}")
...
class ICLightGenerator:
    ...
    def __init__(self, api_url: str = None):
        ...
        if self.api_url:
            print(f"[ICLight] API模式: {self.api_url}")
            _start_local_service(self._session, self.api_url)
        else:
            if _mock_disabled():
                raise RuntimeError("[ICLight] REFLECTVPR_DISABLE_MOCK=1，但未配置 ICLIGHT_API_URL")
            print("[ICLight] 无API配置，使用Mock模式")
    ...
    def generate(
        ...
        **kwargs,
        ...
        if not self.api_url:
            if _mock_disabled():
                raise RuntimeError("[ICLight] REFLECTVPR_DISABLE_MOCK=1，禁止无API时回退Mock")
        ...
        try:
            ...
            if _mock_disabled():
                raise RuntimeError(f"[ICLight] API调用失败，严格模式禁止回退Mock: {e}") from e
            print(f"[ICLight] API调用失败 ({e}), 回退到Mock")
            return self._mock_generate(ref_image, prompt)
        finally:
...
if __name__ == "__main__":
    ...
    print(f"[ICLight] 输出尺寸: {result.size}")
    result.save("/tmp/iclight_test.jpg")
    print("[ICLight] 保存至 /tmp/iclight_test.jpg")
